...
//!   }
//!
//!   let filter = BlockFilter::compute_script_filter (&block, get_script_for_coin)
//!
//!   read and evaluate a filter
//!
//!   let query: Iterator<Item=Script> = // .. some scripts you care about
//!   let mut reader = // .. a single pass reader of binary filter data
//!   if BlockFilter::match_any (&block_hash, &mut reader, &mut query.map(|s| s.as_bytes())) {
//!     // get this block
//!   }
//!
//!
...
use bitcoin_hashes::{sha256d, Hash, siphash24};
...
impl Error for BlockFilterError {
    fn description(&self) -> &str {
        "unresolved UTXO"
    }
}

impl Display for BlockFilterError {
    fn fmt(&self, f: &mut Formatter) -> Result<(), fmt::Error> {
        match *self {
            BlockFilterError::UTXOMissing(ref coin) => write!(f, "unresolved UTXO {}", coin),
            BlockFilterError::IO(ref io) => write!(f, "{}", io)
        }
    }
}

impl Debug for BlockFilterError {
    fn fmt(&self, f: &mut Formatter) -> Result<(), fmt::Error> {
        match *self {
            BlockFilterError::UTXOMissing(ref coin) => write!(f, "unresolved UTXO {}", coin),
            BlockFilterError::IO(ref io) => write!(f, "{:?}", io)
...
impl From<io::Error> for BlockFilterError {
    fn from(io: io::Error) -> Self {
        BlockFilterError::IO(io)
    }
}
...
impl BlockFilter {
    ...
    pub fn compute_script_filter<M>(block: &Block, script_for_coin: M) -> Result<BlockFilter, BlockFilterError>
        where M: Fn(&OutPoint) -> Result<Script, BlockFilterError> {
        ...
        Ok(BlockFilter{block: block.bitcoin_hash(), filter_type: SCRIPT_FILTER, content: out.into_inner()})
    }

    /// match any query pattern
    pub fn match_any (block_hash: &sha256d::Hash, reader: &mut io::Read, query: &mut Iterator<Item=&[u8]>) -> Result<bool, io::Error> {
        let filter_reader = BlockFilterReader::new(block_hash)?;
        filter_reader.match_any(reader, query)
    }

    /// match all query pattern
    pub fn match_all (block_hash: &sha256d::Hash, reader: &mut io::Read, query: &mut Iterator<Item=&[u8]>) -> Result<bool, io::Error> {
        let filter_reader = BlockFilterReader::new(block_hash)?;
        filter_reader.match_all(reader, query)
    }
}
...
impl <'a> BlockFilterWriter<'a> {
    ...
    pub fn add_input_scripts<M>(&mut self, script_for_coin: M) -> Result<(), BlockFilterError>
        where M: Fn(&OutPoint) -> Result<Script, BlockFilterError> {
...
impl BlockFilterReader {
    ...
    pub fn new (block_hash: &sha256d::Hash) -> Result<BlockFilterReader, io::Error> {
        ...
        for t in 0..7 {
            ...
            let mut constructed_filter = Cursor::new(Vec::new());
            {
                ...
                writer.add_input_scripts(
                    ...
                    Err(BlockFilterError::UTXOMissing(o.clone()))
                }).unwrap();
